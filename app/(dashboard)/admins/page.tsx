...
function AdminDetailsModal({ admin, onClose, onBlock }: AdminDetailsModalProps) {
  return (
    <div
...
function BlockConfirmModal({ onCancel, onConfirm }: BlockConfirmModalProps) {
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Confirm block admin"
      onClick={onCancel}
    >
      <div
    ...
    <>
      <section className="overflow-hidden rounded-[0.75rem] border border-[#14274c]/10 bg-[#f3f4f8] shadow-[0_8px_20px_rgba(35,47,78,0.08)]">
        <header className="flex flex-wrap items-center justify-between gap-3 bg-[#16254a] px-4 py-3 sm:px-5">
          <h1 className="m-0 text-[2rem] font-semibold leading-tight text-[#d5dceb] sm:text-[2.15rem]">Admin List</h1>

          <div className="flex w-full flex-wrap items-center gap-3 sm:w-auto">
            <label className="relative min-w-[14rem] flex-1 sm:w-[17rem] sm:flex-none" htmlFor="admin-search">
            ...
            <Link
              href="/admins/blocked-admins"
              className="inline-flex h-[2.55rem] items-center rounded-[0.35rem] border border-white bg-white px-6 text-[0.86rem] font-semibold text-[#22d3c5] transition-colors hover:bg-[#f8fafc] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#44e9e1]"
            >
              Blocked Admins
            </Link>
          </div>
        ...
        <div className="p-4 sm:p-5">
          <div className="mb-3 flex justify-end">
            <label className="inline-flex items-center gap-2">
              ...
              <select
                aria-label="Sort admins by date"
                value={dateSort}
